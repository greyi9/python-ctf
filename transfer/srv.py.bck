...
import imp
def import_modules_in(imp_path):
    m = []
    names = os.listdir(imp_path)
    print os.path.splitext(imp_path + "/login.py")
    for name in names:
        extension = os.path.splitext(imp_path + "/" + name)[1]
        if (extension == ".py" and 'init' not in name):  
            m.append(imp.load_source(name, imp_path + "/" + name))
    return m
modules = import_modules_in(PY_DIR)
...
def handle_pyfile(file_requested, method, input):
    ...
    try:
        return m.process_request(method, input)
    except Exception as e:
        return e
...
def get_requested_module(file_requested):
    ...
    ff = PY_DIR + "/" + f
    requested_module_py = '<module \'%s.py\' from \'%s\'>' % (f,ff + ".py")
    requested_module_pyc = '<module \'%s.py\' from \'%s\'>' % (f,ff + ".pyc")
    for m in modules:
        if (requested_module_py == str(m) or requested_module_pyc == str(m)):
            return m
